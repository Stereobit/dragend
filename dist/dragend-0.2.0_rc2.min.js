...
for(d=d.replace(/^[a-z]/,function(a){return a.toUpperCase()});c--;)if(b[c]+d in a.style)return!0;return!1}}(),p=function(a,b){var c=g({},l);this.settings=g(c,b);this.container=a;this.pageContainer=document.createElement("div");this.scrollBorder={x:0,y:0};this.page=0;this.preventScroll=!1;this.pageCssProperties={margin:0};this.pageContainer.innerHTML=this.container.innerHTML;this.container.innerHTML=null;this.container.appendChild(this.pageContainer);h(this.container,r);console.log(this);this._observe();
f.setTimeout(k(function(){this.updateInstance(b)},this),10)},u=function(a){"horizontal"===this.settings.direction?h(this.pageContainer,{"-webkit-transform":"translateX("+a.x+"px)","-moz-transform":"translateX("+a.x+"px)","-ms-transform":"translateX("+a.x+"px)","-0-transform":"translateX("+a.x+"px)",transform:"translateX("+a.x+"px)"}):"vertical"===this.settings.direction&&h(this.pageContainer,{"-webkit-transform":"translateY("+a.y+"px)","-moz-transform":"translateY("+a.y+"px)","-ms-transform":"translateY("+
a.y+"px)","-o-transform":"translateY("+a.y+"px)",transform:"translateY("+a.y+"px)"})},w=function(){h(this.pageContainer,{"-webkit-transition":"-webkit-transform "+this.settings.duration+"ms ease-out","-moz-transition":"-moz-transform "+this.settings.duration+"ms ease-out","-ms-transition":"-ms-transform "+this.settings.duration+"ms ease-out","-o-transition":"-o-transform "+this.settings.duration+"ms ease-out",transition:"transform "+this.settings.duration+"ms ease-out"});this._scroll({x:-this.scrollBorder.x,
y:-this.scrollBorder.y});f.setTimeout(k(v,this),this.settings.duration)},v=function(){this._onSwipeEnd();h(this.pageContainer,{"-webkit-transition":"-webkit-transform 0"})},x=function(){var a,b;this.activeElement=this.pages[this.page*this.settings.itemsInPage];"horizontal"===this.settings.direction?(a="marginLeft",b=-this.scrollBorder.x):"vertical"===this.settings.direction&&(a="marginTop",b=-this.scrollBorder.y);s(this.pageContainer,a,b,this.settings.duration,k(this._onSwipeEnd,this))};(function(){var a,
b;t("transform")?(a=u,b=w):(a=_scrollWithoutTransform,b=x);g(p.prototype,{_scroll:a,_animateScroll:b})})();g(p.prototype,{_checkOverscroll:function(a,b,c){var d={x:b,y:c,overscroll:!0};switch(a){case "right":if(!this.scrollBorder.x&&this._checkGestureDirection(a))return Math.round(d.x=(b-this.scrollBorder.x)/5),d;break;case "left":if((this.pagesCount-1)*this.pageDimentions.width<=this.scrollBorder.x)return d.x=Math.round(-((Math.ceil(this.pagesCount)-1)*(this.pageDimentions.width+this.settings.borderBetweenPages))+
