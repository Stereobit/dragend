...
"px)",transform:"translateY("+a.y+"px)"})},v=function(){g(this.pageContainer,{"-webkit-transition":"-webkit-transform "+this.settings.duration+"ms ease-out","-moz-transition":"-moz-transform "+this.settings.duration+"ms ease-out","-ms-transition":"-ms-transform "+this.settings.duration+"ms ease-out","-o-transition":"-o-transform "+this.settings.duration+"ms ease-out",transition:"transform "+this.settings.duration+"ms ease-out"});this._scroll({x:-this.scrollBorder.x,y:-this.scrollBorder.y});f.setTimeout(k(afterScrollTranslate,
this),this.settings.duration)},w=function(a){"horizontal"===this.settings.direction?g(this.pageContainer,{marginLeft:a.x}):"vertical"===this.settings.direction&&g(this.pageContainer,{marginTop:a.y})},x=function(){var a,b;this.activeElement=this.pages[this.page*this.settings.itemsInPage];"horizontal"===this.settings.direction?(a="marginLeft",b=-this.scrollBorder.x):"vertical"===this.settings.direction&&(a="marginTop",b=-this.scrollBorder.y);s(this.pageContainer,a,b,this.settings.duration,k(this._onSwipeEnd,
this))};(function(){var a,b;t("transform")?(a=u,b=v):(a=w,b=x);h(p.prototype,{_scroll:a,_animateScroll:b})})();h(p.prototype,{_checkOverscroll:function(a,b,c){var d={x:b,y:c,overscroll:!0};switch(a){case "right":if(!this.scrollBorder.x&&this._checkGestureDirection(a))return Math.round(d.x=(b-this.scrollBorder.x)/5),d;break;case "left":if((this.pagesCount-1)*this.pageDimentions.width<=this.scrollBorder.x)return d.x=Math.round(-((Math.ceil(this.pagesCount)-1)*(this.pageDimentions.width+this.settings.borderBetweenPages))+
b/5),d;break;case "down":if(!this.scrollBorder.y&&this._checkGestureDirection(a))return d.y=Math.round((c-this.scrollBorder.y)/5),d;break;case "up":if((this.pagesCount-1)*this.pageDimentions.height<=this.scrollBorder.y)return d.y=Math.round(-((Math.ceil(this.pagesCount)-1)*(this.pageDimentions.height+this.settings.borderBetweenPages))+c/5),d}return{x:b-this.scrollBorder.x,y:c-this.scrollBorder.y,overscroll:!1}},_observe:function(){if(!m)throw Error("It seems like Hammer JS is not be included before dragend JS");
