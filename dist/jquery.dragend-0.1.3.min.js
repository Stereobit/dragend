...
this.setContainerCssValues();c.extend(this.pageCssProperties,{height:this.pageDimentions.height,width:this.pageDimentions.width});this.pages.css(this.pageCssProperties);void 0!==this.settings.scrollToPage?(this._scrollToPage("page",this.page),delete this.settings.scrollToPage):this._jumpToPage("page",this.page)},_calcNewPage:function(a,b){switch(a){case "up":this.page<this.pages.length-1&&(this.scrollBorder.y=this.scrollBorder.y+this.pageDimentions.height+this.settings.borderBetweenPages,this.page++);
break;case "down":0<this.page&&(this.scrollBorder.y=this.scrollBorder.y-this.pageDimentions.height-this.settings.borderBetweenPages,this.page--);break;case "left":this.page<this.pages.length-1&&(this.scrollBorder.x=this.scrollBorder.x+this.pageDimentions.width+this.settings.borderBetweenPages,this.page++);break;case "right":0<this.page&&(this.scrollBorder.x=this.scrollBorder.x-this.pageDimentions.width-this.settings.borderBetweenPages,this.page--);break;case "page":switch(this.settings.direction){case "horizontal":this.scrollBorder.x=
...
_scrollToPage:function(a,b){this.preventScroll=!0;a&&this._calcNewPage(a,b);this._animateScroll()},swipe:function(a){this.activeElement=this.pages.eq(this.page);this.settings.onSwipeStart(this.container,this.activeElement,this.page);this._scrollToPage(a)},updateInstance:function(a){"object"===typeof a&&c.extend(this.settings,a);void 0!==this.settings.jumpToPage?this.page=this.settings.jumpToPage:void 0!==this.settings.scrollToPage&&(this.page=this.settings.scrollToPage);this.pages=this.container.find(this.settings.pageElements);
this.activeElement=this.pages.eq(this.page);this._sizePages()}});c.fn.dragend=function(a){var b=this.data("dragend");b?b.updateInstance(a):(b=new e(this,a),this.data("dragend",b));"string"===typeof a&&b.swipe(a);return this}})(jQuery,window);
